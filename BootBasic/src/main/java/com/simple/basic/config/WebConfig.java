...
@PropertySource("classpath:/application-production.properties") //classpath:/ 리소스 이하파일을 가리킴
public class WebConfig implements WebMvcConfigurer {
    ...
    @Bean //스프링이 이멧드를 호출 시켜서 반환되는 값을 bean으로 등록시킴
    public void myTest(){
    ...
    */
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        registry.addInterceptor(userAuthHandler())
                ...
                .excludePathPatterns("/user/login","/user/loginForm"); //login페이지는 제외함..

        //만약 인터셉터가 여러개라면 밑에서 또 추가하면 됨.

//        registry.addInterceptor(인터셉터객체)
//                .addPathPatterns("적용할경로")
