...
public class ProductServiceImpl implements ProductService {
    ...
    //@Transactional - 이 메서드를 하나의 트랜잭션을 관리함.
    @Transactional(rollbackFor = Exception.class) //에러가 터지면 롤백 (try~catch로 에러처리가 되면 롤백x)
    @Override
    public int productRegist(ProductVO vo, List<MultipartFile> files) {
        ...
        for(MultipartFile file : files){
            ...
            //generateKey - 인서트 넣기전에 인서트한 데이터의 키값을 조회
            ...
            productMapper.productRegistFile(
                    ...
                    .filepath(filepath)
                    .uuid(uuid.toString())
                    .prodId(vo.getProdId()) //generateKey로
