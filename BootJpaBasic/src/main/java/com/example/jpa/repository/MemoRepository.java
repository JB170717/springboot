...
public interface MemoRepository extends JpaRepository<Memo, Long>
    ...
    //마지막 매개변수에 Pagealbe을 넣게되면 페이지 처리를 합니다. (반환도 page타입)
    //where text like ? or writer list ?
    ...
    //JPQL - update구문 - @Modifying, @Transactional을 반드시 적어줍니다
    //기본구문-update 테이블명 set ~~ where 키=?
    //update, delete는 @Modifying, @Transactional을 꼭 붙여야함..
    @Modifying
    @Transactional
    ...
    public int deleteMemo(@Param("a") Long mno);

    //JPQL구문에 맨 마지막에 pageable을 넣으면 페이지 처리를 합니다.
    @Query("select m from Memo m where m.mno >= :a")
    Page<Memo> getListPage(@Param("a") Long mno, Pageable pageable); //맨마지막 매개변수에 Pageable객체넣으면 페이지처리..
